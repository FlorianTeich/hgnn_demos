...
def copy_nodes_to_kuzu(
    ...
    conn.execute(
        "COPY "
        + tablename
        + ' FROM ("%s",  "%s", "%s") BY COLUMN;'
        % (ids_path, node_feature_path, node_label_path)
    )


def create_kuzu_relationship_table(
    conn, relationshipname="cites", from_="papers", to_="papers"
):
    conn.execute(
        "CREATE REL TABLE "
        + relationshipname
...
def kuzu_node_table_from_arrays(
    conn,
    tablename="paper",
    feats=None,
    labels=None,
...
def kuzu_edges_from_tensor(
    ...
    create_kuzu_relationship_table(
        conn, relationshipname=relationshipname, from_=from_, to_=to_
    )
