"""Manage the training and evaluation of the models"""
...
def migrate_data_to_kuzu(yamlfile="default.yml"):
    ...
    for nodetype in config["nodes"]:
        ...
        utils.kuzu_node_table_from_arrays(
            conn,
    ...
    for edgetype in config["edges"]:
        ...
        edges = func()
        utils.kuzu_edges_from_tensor(
            conn, edges, edgetype["name"], edgetype["from"], edgetype["to"]
        )
...
def train_test_loop(yamlfile="default.yml"):
    ...
    train_inds = torch.tensor(
        pd.read_parquet(config["task"]["train_inds"])["ids"].values, dtype=torch.long
    )
    test_inds = torch.tensor(
        pd.read_parquet(config["task"]["test_inds"])["ids"].values, dtype=torch.long
    )
    ...
    def train(epoch=0):
        ...
        for btch in train_loader:
            try:
                ...
                optimizer.step()
                preds.extend(
                    torch.argmax(torch.softmax(out, dim=-1), dim=-1).detach().tolist()
                )
...
def prepare_transformer_benchmark():
    ...
    diagnosis_agg = (
        diagnosis_data.loc[:, ["PID", "DiagnosisCode", "diagnosis_age"]]
        .groupby("PID")
        .agg(lambda x: "".join(x.str[0]))
    )
    drug_data["drug_age"] = drug_data["drug_age"].astype(str)
    drug_agg = (
        drug_data.loc[:, ["PID", "DrugCode", "drug_age"]]
        .groupby("PID")
        .agg(lambda x: "".join(x.str[0]))
    )

    final_df = (
        persons.loc[:, ["PID", "label", "mode"]]
        .merge(diagnosis_agg, on="PID")
        .merge(drug_agg, on="PID")
    )
    ...
    diagnosis_hist = [
        " ".join(
            [
                "$" + i + j
                for i, j in zip(
                    final_df["DiagnosisCode"].values[k],
                    final_df["diagnosis_age"].values[k],
                )
            ]
        )
    ...
    drug_hist = [
        " ".join(
            [
                "#" + i + j
                for i, j in zip(
                    final_df["DrugCode"].values[k], final_df["drug_age"].values[k]
                )
            ]
        )
        for k in range(len(final_df))
    ...
    final_df.to_parquet(
        str(pathlib.Path(__file__).parent.parent.resolve()) + "/data/final.parquet"
    )
...
def benchmark_transformer():
    ...
    cmodel = AutoModelForSequenceClassification.from_pretrained(
        modelname, config=settings
    )
    ...
    scheduler = torch.optim.lr_scheduler.LinearLR(
        optimizer, start_factor=0.5, total_iters=epochs
    )
    ...
    tokenized_datasets_train = Dataset.from_pandas(
        final_df[final_df["mode"] == "train"].loc[:, ["label", "history"]],
        preserve_index=False,
    ).map(tokenize_function, batched=True)
    tokenized_datasets_test = Dataset.from_pandas(
        final_df[final_df["mode"] == "test"].loc[:, ["label", "history"]],
        preserve_index=False,
    ).map(tokenize_function, batched=True)
    ...
    for e in range(epochs):
        ...
        for i in range(len(test_res[-1]["test_preds"])):
            acc += (
                test_res[-1]["test_preds"][i]["logits"].argmax(1)
                == test_res[-1]["test_targets"][i]
            ).sum()
        print(acc)
